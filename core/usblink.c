...
enum SID { SID_File = 0x8001, SID_Dirlist };
...
static uint32_t packet_max_datasize() {
  // TODO: Would be 1440 for NNSE, but usb_cx2 only handles 1023 per transfer
  // max.
...
uint16_t usblink_data_checksum(struct packet *packet) {
  ...
  for (i = 0; i < size; i++) {
    ...
    check =
        (packet->fulldata[i] << 8 | check >> 8) ^ tmp ^ tmp >> 5 ^ tmp >> 12;
  }
...
static void dump_packet(char *type, const void *data, uint32_t size) {
  if (log_enabled[LOG_USB]) {
    ...
    emuprintf("%s", type);
    for (i = 0; i < size; i++)
      emuprintf(" %02x %c", ((uint8_t *)data)[i],
                isprint(((uint8_t *)data)[i]) ? ((uint8_t *)data)[i] : '?');
    emuprintf("\n");
  }
}
...
void usblink_send_packet() {
  ...
  dump_packet("send", &usblink_send_buffer,
              16 + packet_fulldatasize(&usblink_send_buffer));
  usblink_start_send();
}
...
void put_file_next(struct packet *in) {
  ...
  case RECVING_04:
    ...
    if (data[0] != File_Ready) {
      emuprintf("File send error: Didn't get 04, got %02X\n", data[0]);
      if (data[0] == 0xFF && size > 1) {
        status = -data[1];
        emuprintf("Error code: %02X\n", data[1]);
      }
      goto fail;
    }
  ...
  send_data:
    ...
    if (put_file_size > 0) {
      ...
      if (current_file_callback) {
        ...
        // Not 100 as the completion is signaled seperately and mustn't happen
        // more than once
        int progress =
            ((put_file_size_orig - put_file_size) * 99) / put_file_size_orig;
  ...
  case 0x05: // Receive data packet
    ...
    if (in->data_size < 254 || put_file_size == put_file_size_orig) {
      ...
      *data++ = 0x00;
      ...
      if (current_file_callback)
        current_file_callback(put_file_size == put_file_size_orig ? 100 : -1,
                              current_user_data);
    }
  }
}

enum Success_Action { Success_Done, Success_Next } dirlist_success_action;

void usblink_dirlist(const char *dir, usblink_dirlist_cb callback,
                     void *user_data) {
  mode = Dirlist;
...
void dirlist_next(struct packet *in) {
  ...
  if (in->data[0] == 0xFF) // Status message
  {
    ...
    case 0:
      ...
      break;
    case 0x11:
      // It is possible that NavNet answers File_Dirlist_Done with FF 11
      // somehow. Ignore that..
...
void usblink_received_packet(const uint8_t *data, uint32_t size) {
  ...
  if (in && in->src.service == BSWAP16(0x4003)) { /* Address request */
    ...
    memcpy(out->data + 0, &tmp,
           sizeof(tmp)); // *(uint16_t *)&out->data[0] = DST_ADDR;
    tmp = BSWAP16(0xFF00);
    memcpy(out->data + 2, &tmp,
           sizeof(tmp)); // *(uint16_t *)&out->data[2] = BSWAP16(0xFF00);
    usblink_send_packet();
    return;
  } else if (in && !in->ack &&
             !emulate_cx2) { // CX2 acks are handled on the NNSE layer
    /* Send an ACK */
    ...
    out->seqno = in->seqno;
    memcpy(
        &out->data[0], &in->dst.service,
        sizeof(
            in->dst.service)); // *(uint16_t *)&out->data[0] = in->dst.service;
    usblink_send_packet();
  }

  /* Ignore disconnects from the LOGIN service */
  if (in && in->src.service == BSWAP16(0x40DE) &&
      in->dst.service == BSWAP16(0x4050))
    return;
  ...
  case Delete:
  case Dir_Create:
    if (in && packet_datasize(in) == 2 && packet_dataptr(in)[0] == 0xFF &&
        current_file_callback) {
      uint8_t code = packet_dataptr(in)[1];
      current_file_callback(code == 0x00 ? 100 : -code, current_user_data);
      current_file_callback = NULL;
    }
    break;
  case Dirlist:
...
bool usblink_put_file(const char *local, const char *remote,
                      usblink_progress_cb callback, void *user_data) {
  mode = File_Send;

  char *dot = local ? strrchr(local, '.') : NULL;
  // TODO (thanks for the reminder, Excale :P) : Filter depending on which model
  // is being emulated
  if (dot &&
      (!strcmp(dot, ".tno") || !strcmp(dot, ".tnc") || !strcmp(dot, ".tco") ||
       !strcmp(dot, ".tcc") || !strcmp(dot, ".tmo") || !strcmp(dot, ".tmc") ||
  ...
  *(uint32_t *)data = BSWAP32(put_file_size);
  data += 4;
  out->data_size = data - out->data;
  usblink_send_packet();
  return 1;
...
void usblink_new_dir(const char *path, usblink_progress_cb callback,
                     void *user_data) {
  mode = Dir_Create;
  current_file_callback = callback;
  current_user_data = user_data;
...
bool usblink_send_os(const char *filepath, usblink_progress_cb callback,
                     void *user_data) {
  mode = File_Send;
  current_file_callback = callback;
  current_user_data = user_data;
...
void usblink_move(const char *old_path, const char *new_path,
                  usblink_progress_cb callback, void *user_data) {
  mode = Rename;
  current_file_callback = callback;
  current_user_data = user_data;
...
bool usblink_get_file(const char *path, const char *dest,
                      usblink_progress_cb callback, void *user_data) {
  mode = File_Receive;
  current_file_callback = callback;
  current_user_data = user_data;
...
void usblink_delete(const char *path, bool is_dir, usblink_progress_cb callback,
                    void *user_data) {
  mode = Delete;
  current_file_callback = callback;
  current_user_data = user_data;
...
void usblink_start_send() {
  if (emulate_cx2) {
    // Wrap it in NNSE
    usblink_cx2_send_navnet((const uint8_t *)&usblink_send_buffer,
                            16 + packet_fulldatasize(&usblink_send_buffer));
    return;
  }
