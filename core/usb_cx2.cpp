...
static void usb_cx2_send_zlp(uint8_t ep) {
  // Send a zero-length packet on the specified endpoint
  // This signals end-of-transfer to the device
  uint8_t fifo = (usb_cx2.epmap[ep > 4] >> (8 * ((ep - 1) & 0b11) + 4)) & 0b11;

  // Mark FIFO as having received a short packet (ZLP is the ultimate short
  // packet)
...
static bool usb_cx2_real_packet_to_calc(uint8_t ep, const uint8_t *packet,
                                        size_t size) {
  uint8_t fifo = (usb_cx2.epmap[ep > 4] >> (8 * ((ep - 1) & 0b11) + 4)) & 0b11;

  // +1 to adjust for the size increment below (Nspire OS expects this)
  ...
  /* The nspire OS expects packets that are 64-byte aligned to have +1 byte
   * to avoid having to deal with zero-length packets. */
  if ((size & 0x3F) == 0)
    ++size;
  ...
  if (size % packet_size)                     // Last pkt is short?
    usb_cx2.gisr[1] |= 1 << ((fifo * 2) + 1); // FIFO SPK IRQ
  // Note: ZLP handling is now done at the register level (0x150/0x154)
...
static void usb_cx2_packet_from_calc(uint8_t ep, uint8_t *packet, size_t size) {
  if (ep != 1 && ep != 0) // Allow EP0 too?
    error("Got packet on unknown EP");

  if (USBIPServer::instance().isRunning()) {
    USBIPServer::instance().onPacketFromCalc(ep, packet, size);
    return;
  }
...
void usb_cx2_fdma_update(int fdma) {
  ...
  if (fromMemory) {
    ...
    if (usb_cx2.fifo[fifo].size)
      memmove(usb_cx2.fifo[fifo].data, usb_cx2.fifo[fifo].data + length,
              usb_cx2.fifo[fifo].size);
    else {
      usb_cx2.gisr[1] &= ~(0b11 << (fifo * 2)); // FIFO0 OUT/SPK

      if (ep == 1 && send_queue.size()) {
        usb_cx2_packet_to_calc(1, send_queue.front().data,
...
uint32_t usb_cx2_read_word(uint32_t addr) {
  ...
  case 0x124: // DCCPARAMS: Host capable, device capable, 4 endpoints
    return 0x00000184;
  case 0x130:
...
void usb_cx2_write_word(uint32_t addr, uint32_t value) {
  ...
  case 0x114:
    ...
    return;
  case 0x120: {           // DCFESR (Cx config and status)
    if (value & (1 << 3)) // CX_CLR: Clear CX FIFO
      usb_cx2.cxfifo.size = 0;

    if (value & (1 << 2)) { // CX_STALL: Stall CX FIFO
      // Just clear the FIFO and reset state
      usb_cx2.cxfifo.size = 0;
    }

    if (value & (1 << 1)) { // CX_TEST_DONE: Test transfer finished
                            // No-op for now, just don't crash
    }

    if (value & (1 << 5)) { // CX_DONE: CX configuration done / Reset
      // If there is data in the CX FIFO, this indicates the OS is done writing
      // the response (e.g. Device Descriptor) and wants to send it.
      if (usb_cx2.cxfifo.size > 0) {
        usb_cx2_packet_from_calc(0, usb_cx2.cxfifo.data, usb_cx2.cxfifo.size);
      }

      usb_cx2.cxfifo.size = 0;
      usb_cx2.gisr[0] &= ~0b111; // Clear EP0 IRQs
    }

    if (value & (1 << 0)) // CX_CONF_SET: Setup transfer finished
    {
  ...
  case 0x14c:
    ...
    usb_cx2_int_check();
    return;
  case 0x150: // Rx zero length pkt (write-1-to-clear)
    usb_cx2.rxzlp &= ~value;
    usb_cx2_int_check();
    return;
  case 0x154: // Tx zero length pkt (trigger ZLP transmission)
    // For each bit set, send a ZLP on that endpoint
    for (int ep = 1; ep <= 8; ++ep) {
      if (value & (1 << (ep - 1))) {
        usb_cx2_send_zlp(ep);
      }
    }
    usb_cx2.txzlp = 0; // Auto-clears after transmission
    usb_cx2_int_check();
    return;
  case 0x160:
  case 0x164:
  ...
  case 0x1D0: // CX FIFO PIO register
    if (usb_cx2.cxfifo.size < sizeof(usb_cx2.cxfifo.data)) {
      // OS writes byte-wise or word-wise to fill the EP0 IN buffer (cxfifo).
      // Standard behavior is filling the FIFO before triggering the send via
      // CX_DONE.
      uint8_t bytes[4];
      memcpy(bytes, &value, 4);
      for (int i = 0; i < 4; ++i) {
        if (usb_cx2.cxfifo.size < sizeof(usb_cx2.cxfifo.data)) {
          usb_cx2.cxfifo.data[usb_cx2.cxfifo.size++] = bytes[i];
        }
      }
    }
    return;
